...
impl timestamp::Service for TimestampOracle {
    ...
    async fn get_timestamp(&self, _: TimestampRequest) -> labrpc::Result<TimestampResponse> {
        ...
        Ok(TimestampResponse {
            ts: self.logic_timer.fetch_add(1, Ordering::Relaxed)
        })
    }
}
...
impl transaction::Service for MemoryStorage {
    ...
    async fn get(&self, req: GetRequest) -> labrpc::Result<GetResponse> {
        ...
        let ts = req.ts;
        let key = req.key;
        let kv_table = &self.data.lock().unwrap();

        loop {
            if let Some(_) = kv_table.read(key.clone(), Column::Lock, Some(ts), None) {
                self.back_off_maybe_clean_up_lock(ts, key.clone());
                continue;
            }

            if let Some((_, Value::Timestamp(ts))) = kv_table.read(key.clone(), Column::Write, None, Some(ts)) {
                if let Some((_, Value::Vector(v))) = kv_table.read(key.clone(), Column::Data, Some(*ts), Some(*ts)) {
                    return Ok(GetResponse{
                        okay: true, value: v.clone()
                    })
                }
            } 
            return Ok(GetResponse { okay: false, value: vec![] });
        }
    ...
    async fn prewrite(&self, req: PrewriteRequest) -> labrpc::Result<PrewriteResponse> {
        // Your code here.
        let PrewriteRequest{start_ts, key, value, primary_key} = req;
        let kv_table = &mut self.data.lock().unwrap();
        ...
        kv_table.write(key.clone(), Column::Lock, start_ts, Value::Vector(primary_key));
    ...
    async fn commit(&self, req: CommitRequest) -> labrpc::Result<CommitResponse> {
        // Your code here.
        let CommitRequest{is_primary, start_ts, commit_ts, key} = req;
        let kv_table = &mut self.data.lock().unwrap();
        if is_primary {
        ...
        kv_table.write(key.clone(), Column::Write, commit_ts, Value::Timestamp(start_ts));
        kv_table.erase(key, Column::Lock, start_ts);
